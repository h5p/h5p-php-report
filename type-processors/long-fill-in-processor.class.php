...
/**
 * Class LongFillInProcessor
 * Processes and generates HTML report for 'long-fill-in' interaction type.
 */
class LongFillInProcessor extends TypeProcessor {
  ...
  public function generateHTML($description, $crp, $response, $extras, $scoreSettings = NULL) {
    ...
    $this->setStyle('styles/long-fill-in.css');
    ...
    $container =
      '<div class="h5p-reporting-container h5p-long-fill-in-container">' .
        $header . $report .
      '</div>';

    // Footer only required if there is a correct responses pattern
    $footer = (isset($crp) && sizeof($crp) !== 0) ? $this->generateFooter() : '';
  ...
  private function generateHeader($scoreSettings) {
    ...
    return
      "<div class='h5p-long-fill-in-header'>" .
        $scoreHtml .
      "</div>";
  }
  ...
  function generateFooter() {
    return
      '<div class="h5p-long-fill-in-footer">' .
        '<span class="h5p-long-fill-in-correct-responses-pattern">Correct Answer</span>' .
        '<span class="h5p-long-fill-in-user-response-correct">Your correct answer</span>' .
        '<span class="h5p-long-fill-in-user-response-wrong">Your incorrect answer</span>' .
      '</div>';
  }
  ...
  private function getPlaceholderReplacements($crp, $response, $caseSensitive) {
    ...
    if (sizeof($crp) === 0) {
      foreach($response as $answer) {
        $placeholderReplacements[] =
          '<p class="h5p-long-fill-in-user-response h5p-long-fill-in-user-response-correct h5p-long-fill-in-no-correct">' .
          $answer .
          '</p>';
      }
    }
    ...
    foreach ($crp as $index => $value) {
      ...
      $responseClass = $isCorrect ?
        'h5p-long-fill-in-user-response-correct' :
        'h5p-long-fill-in-user-response-wrong';

      // Format the placeholder replacements
      $userResponse =
        '<p class="h5p-long-fill-in-user-response ' . $responseClass . '">' .
        $currentResponse .
        '</p>';
      ...
      if (strlen($CRPhtml) > 0) {
        $correctResponsePattern .=
          '<p class="h5p-long-fill-in-correct-responses-pattern">' .
            $CRPhtml .
          '</p>';
      }
